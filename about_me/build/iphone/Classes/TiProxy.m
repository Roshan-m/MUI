...
//Rare exceptions to indicate a bug in the _about_me code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_about_me = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_about_me]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.about_me.module"
