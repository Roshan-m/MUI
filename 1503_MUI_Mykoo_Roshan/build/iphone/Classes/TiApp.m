...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1503_MUI_Mykoo_RoshanUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1503_MUI_Mykoo_RoshanUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1503_MUI_Mykoo_Roshan app using the node.js CLI
