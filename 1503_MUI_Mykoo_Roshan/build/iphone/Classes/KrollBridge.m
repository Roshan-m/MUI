...
NSString * k1503_MUI_Mykoo_Roshan$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1503_MUI_Mykoo_RoshanObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1503_mui_mykoo_roshan);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k1503_MUI_Mykoo_Roshan global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1503_MUI_Mykoo_Roshan" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1503_mui_mykoo_roshan = [[k1503_MUI_Mykoo_RoshanObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1503_mui_mykoo_roshan];
	
	NSString *_k1503_mui_mykoo_roshanNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1503_mui_mykoo_roshanNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1503_mui_mykoo_roshan valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1503_MUI_Mykoo_Roshan$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1503_mui_mykoo_roshan.kroll" 
